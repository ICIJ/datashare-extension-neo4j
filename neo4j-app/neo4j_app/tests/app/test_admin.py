...
from neo4j_app.core.objects import Neo4jCSVResponse, NodeCSVs, RelationshipCSVs
...
def test_post_named_entities_import_should_return_200(
    ...
    expected_response = Neo4jCSVResponse(
        nodes=[
            NodeCSVs(
                labels=["Document"],
                header_path="docs-header.csv",
                node_paths=["docs.csv.gz"],
                n_nodes=1,
            ),
            NodeCSVs(
                labels=[],
                header_path="entities-header.csv",
                node_paths=["entities.csv.gz"],
                n_nodes=2,
            ),
        ],
        relationships=[
            RelationshipCSVs(
                ...
                relationship_paths=["doc-roots.csv.gz"],
                n_relationships=0,
            ),
            RelationshipCSVs(
                types=["APPEARS_IN"],
                header_path="entity-docs-header.csv",
                relationship_paths=["entity-docs.csv.gz"],
                n_relationships=2,
            ),
        ],
    )
